					...
					<div className="item-header">
						<div className="author">
							<div className="avator" >
								<Avatar size={42} src={item.user.avatar} />
							</div>
						</div>
						<div className="info">
							<div className="name">
								<span className="blue">{item.user.name} {item.user.type === 0 ? '(作者)' : ''}</span>
							</div>
							<div className="time">
								<Icon type="environment" size="14" twoToneColor="#969696"/>&nbsp;{item.map_marker || 'CHINA'}&nbsp;&nbsp;&nbsp;{item.create_time ? timestampToTime(item.create_time,true) : ''}
							</div>
						</div>
					</div>
						...
						return (
							<div key={e._id} className="item-other">
								<div className="item-header">
									<div className="author">
										<div className="avator">
											<Avatar size={38} src={e.user.avatar} />
										</div>
									</div>
									<div className="info">
										<div className="name">
											<span className="blue">{e.user.name} {e.user.type === 0 ? '(作者)' : ''}</span>
											&nbsp;回复&nbsp;
											<span className="blue">{e.to_user.name} {e.to_user.type === 0 ? '(作者)' : ''}</span>：
										</div>
										<div className="time">
											<Icon type="environment" size="14" twoToneColor="#969696"/>&nbsp;{e.map_marker || 'CHINA'}&nbsp;&nbsp;&nbsp;{e.create_time ? timestampToTime(e.create_time,true) : ''}
										</div>
									</div>
								</div>
								<div className="comment-detail">
